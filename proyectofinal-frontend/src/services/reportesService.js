...
const reportesService = {
  ...
  async generateReport(fecha_inicio, fecha_fin, formato = 'pdf') {
    try {
      const response = await apiClient.post('/api/reportes/generate', {
        fecha_inicio,
        fecha_fin,
        formato
      });
  ...
  async getMyReports() {
    try {
      const response = await apiClient.get('/api/reportes/my-reports');
      return response.data;
    } catch (error) {
      throw new Error(
        error.response?.data?.message || 
  ...
  async getReportById(id_reporte) {
    try {
      const response = await apiClient.get(`/api/reportes/${id_reporte}`);
      return response.data;
    } catch (error) {
      throw new Error(
        error.response?.data?.message || 
